...
import dev.vikel.taskmanagement.users.UserEntity;
...
public class TaskController {
    ...
    public ResponseEntity<Task> updateTask(@PathVariable String userId, @PathVariable String taskId, @RequestBody Task updatedTask){
    ...
    public ResponseEntity<Task> createTask(@PathVariable String userId, @RequestBody Task newTaskInfo){
        ...
        Optional<UserEntity> existingUser = userService.findUser(userId);
    ...
    public ResponseEntity<String> deleteTask(@PathVariable String userId, @PathVariable String taskId){
        Optional<UserEntity> existingUser = userService.findUser(userId);
        Optional<Task> existingTask = taskService.specificTask(taskId);
        
        //Using .get() for comparison
        Task checkTask = existingTask.get();
        UserEntity checkUser = existingUser.get();
